...
export function Money({
  cents, locale, signed = false,
}: { cents: number; locale: Locale; signed?: boolean }) {
  return (
...
export function EmptyState({ text }: { text: string }) {
  return (
    <div className="empty-state">
      <ReceiptText size={28} /><span>{text}</span>
    </div>
  )
}
...
export function Modal({
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <div
        className="modal-card" role="dialog" aria-modal="true" aria-label={title}
