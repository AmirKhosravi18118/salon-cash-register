...
import type {
  AppSetting, CashSession, CashTransaction, DashboardSnapshot,
  PriceHistory, SaleItem, SalonService, ServiceKind, TransactionKind,
} from './types'
...
const starterServices: Array<Omit<SalonService, 'id' | 'createdAt' | 'updatedAt'>> = [
  {
    kind: 'package', nameFa: 'شست‌وشو + کوتاهی + براشینگ',
    nameDe: 'Waschen + Schneiden + Föhnen',
    categoryFa: 'پکیج محبوب', categoryDe: 'Beliebtes Paket',
    priceCents: 5500, active: true, allowCustomPrice: false, accent: colors[0],
  },
  {
    kind: 'package', nameFa: 'رنگ ریشه + براشینگ',
    nameDe: 'Ansatzfarbe + Föhnen',
    categoryFa: 'پکیج رنگ', categoryDe: 'Farbpaket',
    priceCents: 6900, active: true, allowCustomPrice: true, accent: colors[1],
  },
  {
    kind: 'service', nameFa: 'کوتاهی زنانه', nameDe: 'Damenhaarschnitt',
    categoryFa: 'کوتاهی', categoryDe: 'Schnitt',
    priceCents: 3500, active: true, allowCustomPrice: false, accent: colors[2],
  },
  {
    kind: 'service', nameFa: 'براشینگ', nameDe: 'Föhnen',
    categoryFa: 'حالت‌دهی', categoryDe: 'Styling',
    priceCents: 2800, active: true, allowCustomPrice: true, accent: colors[3],
  },
  {
    kind: 'service', nameFa: 'رنگ ریشه', nameDe: 'Ansatzfarbe',
    categoryFa: 'رنگ مو', categoryDe: 'Farbe',
    priceCents: 4900, active: true, allowCustomPrice: true, accent: colors[4],
  },
  {
    kind: 'service', nameFa: 'رنگ کامل — موی کوتاه',
    nameDe: 'Komplettfarbe — Kurz', categoryFa: 'رنگ مو', categoryDe: 'Farbe',
    priceCents: 6000, active: true, allowCustomPrice: true, accent: colors[5],
  },
  {
    kind: 'service', nameFa: 'رنگ کامل — موی متوسط',
    nameDe: 'Komplettfarbe — Mittel', categoryFa: 'رنگ مو', categoryDe: 'Farbe',
    priceCents: 8000, active: true, allowCustomPrice: true, accent: colors[0],
  },
  {
    kind: 'service', nameFa: 'رنگ کامل — موی بلند',
    nameDe: 'Komplettfarbe — Lang', categoryFa: 'رنگ مو', categoryDe: 'Farbe',
    priceCents: 11000, active: true, allowCustomPrice: true, accent: colors[1],
  },
  {
    kind: 'service', nameFa: 'اصلاح و طراحی ابرو',
    nameDe: 'Augenbrauen formen', categoryFa: 'ابرو', categoryDe: 'Augenbrauen',
    priceCents: 1500, active: true, allowCustomPrice: false, accent: colors[2],
  },
  {
    kind: 'service', nameFa: 'کراتین موی بلند',
    nameDe: 'Keratinbehandlung — Lang', categoryFa: 'احیا و کراتین', categoryDe: 'Keratin',
    priceCents: 18000, active: true, allowCustomPrice: true, accent: colors[3],
  },
]
...
export async function ensureSeedData(): Promise<void> {
  if (await db.services.count()) return

  const now = nowIso()
  await db.services.bulkAdd(starterServices.map((service) => ({
    ...service, id: id(), createdAt: now, updatedAt: now,
  })))
  await db.settings.bulkPut([
    { key: 'sequence', value: '0' },
    { key: 'locale', value: 'fa' },
    { key: 'appVersion', value: '0.4.0-test' },
  ])
  ...
  if (haircut && packageItem && brow) {
    await addTransaction({
      kind: 'sale', amountCents: 4000, tipCents: 500, cashEffectCents: 4000,
      category: 'Services', note: '', sessionId: session.id, isDemo: true,
      items: [{
        serviceId: haircut.id, nameFa: haircut.nameFa, nameDe: haircut.nameDe,
        quantity: 1, unitPriceCents: 3500, totalCents: 3500,
      }],
    })
    await addTransaction({
      kind: 'sale', amountCents: 5800, tipCents: 300, cashEffectCents: 5800,
      category: 'Packages', note: '', sessionId: session.id, isDemo: true,
      items: [{
        serviceId: packageItem.id, nameFa: packageItem.nameFa, nameDe: packageItem.nameDe,
        quantity: 1, unitPriceCents: 5500, totalCents: 5500,
      }],
    })
    await addTransaction({
      kind: 'sale', amountCents: 1500, tipCents: 0, cashEffectCents: 1500,
      category: 'Services', note: '', sessionId: session.id, isDemo: true,
      items: [{
        serviceId: brow.id, nameFa: brow.nameFa, nameDe: brow.nameDe,
        quantity: 1, unitPriceCents: 1500, totalCents: 1500,
      }],
    })
    await addTransaction({
      kind: 'expense', amountCents: 2450, tipCents: 0, cashEffectCents: -2450,
      category: 'Material', note: 'Test / Demo',
      sessionId: session.id, isDemo: true, items: [],
    })
  }
}
...
export async function openCashSession(openingBalanceCents: number): Promise<CashSession> {
  ...
  const session: CashSession = {
    id: id(), status: 'open', openingBalanceCents, openedAt: nowIso(),
  }
...
export async function closeCashSession(countedBalanceCents: number): Promise<CashSession> {
  ...
  const closed: CashSession = {
    ...session, status: 'closed', closedAt: nowIso(),
    expectedBalanceCents: expected, countedBalanceCents,
...
export async function saveSale(input: {
  ...
  return addTransaction({
    kind: 'sale', amountCents: total, tipCents: input.tipCents,
    cashEffectCents: total, category: 'Services',
    items: input.items, sessionId: input.sessionId,
  })
}
  ...
  await db.transaction('rw', db.services, db.priceHistory, async () => {
    if (existing && existing.priceCents !== saved.priceCents) {
      await db.priceHistory.add({
        id: id(), serviceId: saved.id,
        serviceNameFa: saved.nameFa, serviceNameDe: saved.nameDe,
        previousPriceCents: existing.priceCents,
        newPriceCents: saved.priceCents, changedAt: now,
      })
    }
...
export async function createService(kind: ServiceKind): Promise<SalonService> {
  const now = nowIso()
  return {
    id: id(), kind, nameFa: '', nameDe: '', categoryFa: '', categoryDe: '',
    priceCents: 0, active: true, allowCustomPrice: false,
...
export async function exportBackup(): Promise<string> {
  return JSON.stringify({
    exportedAt: nowIso(), version: '0.4.0-test',
...
export async function importBackup(content: string): Promise<void> {
  ...
  if (!Array.isArray(payload.services) || !Array.isArray(payload.transactions)) {
  ...
  await db.transaction(
    ...
    async () => {
      await Promise.all([
        db.services.clear(), db.transactions.clear(), db.sessions.clear(),
        db.priceHistory.clear(), db.settings.clear(),
      ])
      await db.services.bulkPut(payload.services ?? [])
      await db.transactions.bulkPut(payload.transactions ?? [])
      await db.sessions.bulkPut(payload.sessions ?? [])
      await db.priceHistory.bulkPut(payload.priceHistory ?? [])
...
export async function exportTransactionsCsv(): Promise<string> {
  const transactions = await db.transactions.orderBy('createdAt').toArray()
  const rows = [
    ['Sequence', 'Date', 'Type', 'AmountEUR', 'TipEUR',
      'CashEffectEUR', 'Category', 'Note', 'Items'],
    ...transactions.map((transaction) => [
      transaction.sequence, transaction.createdAt, transaction.kind,
      (transaction.amountCents / 100).toFixed(2),
      (transaction.tipCents / 100).toFixed(2),
      (transaction.cashEffectCents / 100).toFixed(2),
      transaction.category, transaction.note,
      transaction.items.map((item) => `${item.nameDe} x${item.quantity}`).join(' | '),
    ]),
  ]
