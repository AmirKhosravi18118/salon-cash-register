import {
  ArrowDownLeft, ArrowUpRight, Check, Euro,
  Landmark, RotateCcw, UserRound, WalletCards,
} from 'lucide-react'
...
export function MovementPage({
  ...
  useEffect(() => { getOpenSession().then(setSession) }, [revision])
  ...
  const submit = async (event: FormEvent) => {
    ...
    await saveCashMovement({
      kind, amountCents, category, note, sessionId: session.id,
    })
    setAmount('')
    setCategory('')
    setNote('')
    ...
    <>
      ...
      {!session && (
        <div className="notice warning">
          <WalletCards size={20} /><strong>{t('movement.openCashboxFirst')}</strong>
      ...
      <GlassCard className="form-card">
        <form onSubmit={submit}>
          <div className="movement-grid">
            {movementKinds.map((item) => {
              const Icon = item.icon
              return (
                <button
                  key={item.kind} type="button"
                ...
                >
                  ...
                  <small>{item.positive ? t('movement.effectIn') : t('movement.effectOut')}</small>
                </button>
              )
            })}
          </div>
            ...
            <label>
              ...
              <input
                ...
                placeholder={kind === 'expense'
                  ? (locale === 'fa' ? 'مثلاً خرید مواد' : 'z. B. Material')
                  : t('common.optional')}
              />
            </label>
          </div>
          ...
          <label>
            ...
            <textarea
              className="input textarea" value={note}
          ...
          <button
            className="primary-button" disabled={!session || parseEuro(amount) <= 0}
            type="submit"
          >
            <Check size={20} />{t('movement.save')}
          </button>
        </form>
      </GlassCard>
