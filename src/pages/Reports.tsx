...
export function ReportsPage({
  ...
  const totalSales = sales.reduce(
    (sum, item) => sum + item.amountCents - item.tipCents, 0,
  )
  ...
  const net = monthTransactions.reduce(
    (sum, item) => sum + item.cashEffectCents, 0,
  )

  const topMap = new Map<string, {
    nameFa: string; nameDe: string; count: number; revenue: number
  }>()
  sales.flatMap((sale) => sale.items).forEach((item) => {
    const current = topMap.get(item.serviceId) ?? {
      nameFa: item.nameFa, nameDe: item.nameDe, count: 0, revenue: 0,
    }
  ...
  return (
    <>
      <PageHeader
        title={t('reports.title')} subtitle={t('reports.subtitle')}
        action={
      ...
      <div className="notice info">
        <Euro size={20} /><strong>{t('reports.cashOnlyNotice')}</strong>
      </div>
