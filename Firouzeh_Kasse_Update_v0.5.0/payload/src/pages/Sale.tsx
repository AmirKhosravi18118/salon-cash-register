...
export function SalePage({
  ...
  const visible = services.filter(
    (service) => filter === 'all' || service.kind === filter,
  )
  ...
  const updateLinePrice = (id: string, value: string) => {
    setCart((current) => current.map((line) =>
      line.service.id === id
        ? { ...line, unitPriceCents: parseEuro(value) }
        : line))
  }
    ...
    <>
      ...
      {!session && (
        <div className="notice warning">
          <WalletCards size={20} />
          <strong>{t('sale.openCashboxFirst')}</strong>
        ...
        <div className="catalog-column">
          <div className="tabs-scroller">
            <div className="segmented catalog-tabs">
              ...
              <button
                className={filter === 'package' ? 'active' : ''}
                type="button"
              ...
              {visible.map((service, index) => (
                <button
                  ...
                  style={{
                    ...
                    '--card-delay': `${Math.min(index * 35, 280)}ms`,
                  } as CSSProperties}
                >
              ...
              {cart.map((line) => (
                <div className="cart-line" key={line.service.id}>
                  ...
                  <div className="quantity-control">
                    <button
                      type="button"
                      onClick={() => updateQuantity(line.service.id, -1)}
                      aria-label="Remove"
                    >
                      −
                    </button>
